...
const cities = [        
    ...
    cities.unshift({name: 'Грозный', population: 328000})
    cities.push({name: 'Курчалой', population: 23425})
    console.log(cities)
...
let arr = cities.filter(function(item){
...
let cities1 = cities.map(function(item){
...
let a = cities1.reduce(function(sum, item){
...
let text1 = text.filter(function(item){
    if(item.id > 5){
        return item
    }
})
